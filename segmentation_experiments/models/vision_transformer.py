...
class ResNet(torch.nn.Module):
    def __init__(self, input_nc, output_nc,
        ...
        if backbone == 'resnet18':
            self.resnet = resnet18(in_channels=input_nc, pretrained=True,
                                          replace_stride_with_dilation=[False,True,True])
        elif backbone == 'resnet34':
            self.resnet = resnet34(in_channels=input_nc, pretrained=True,
                                          replace_stride_with_dilation=[False,True,True])
        elif backbone == 'resnet50':
            self.resnet = resnet50(in_channels=input_nc, pretrained=True,
                                          replace_stride_with_dilation=[False,True,True])
            expand = 4
        else:
...
class ViTSeg(ResNet):
    ...
    def forward(self, x1):
        ...
        if self.token_trans:
            self.tokens_ = torch.cat([token1], dim=1)
            self.tokens = self._forward_transformer(self.tokens_)
        # forward transformer decoder
        if self.with_decoder:
            x = self._forward_transformer_decoder(x1, self.tokens_)
        else:
            x = self._forward_simple_decoder(x1, token1)
